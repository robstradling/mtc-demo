...
[draft-ietf-plants-merkle-tree-certs-06](https://www.ietf.org/archive/id/draft-ietf-plants-merkle-tree-certs-06.txt).
...
|------|-------------|---------|
| `trustanchorid.go` | §5.1 | `TrustAnchorID` type (base-128 OID encoding), `LogID()`, `LandmarkID()` |
...
| `entry.go` | §5.2.1 | Log entry encoding (`MTCLogEntry`) with extensions |
| `cosign.go` | §5.3 | Cosignature generation and verification |
| `cert.go` | §6.2 | `MTCProof` and X.509 certificate construction |
| `verify.go` | §7.1–§7.5 | Relying party verification |
| `log.go` | §5 | `IssuanceLog` management |
| `landmark.go` | §6.4 | `LandmarkSequence` for size-optimized certificates |
...
certDER, _ := mtc.CreateCertificate(
    ...
    extensions,     // MTCLogEntry extensions (or nil)
...
5. Reconstructs the `MTCLogEntry` (with extensions) from the `TBSCertificate`
...
**landmark-relative certificates** (§6.4). These certificates contain only
...
// Each landmark has a tree size and an expiration time (seconds since
// the Epoch). The expiry MUST be >= the notAfter of every entry below
// the tree size, and is typically set to now + max certificate lifetime.
expiry := uint64(time.Now().Add(7 * 24 * time.Hour).Unix())
ls.AllocateLandmark(1000, expiry) // landmark 1 at tree size 1000
ls.AllocateLandmark(5000, expiry) // landmark 2 at tree size 5000
...
// List landmarks that are active (unexpired) at the current time.
active := ls.ActiveLandmarks(uint64(time.Now().Unix()))
```

### Subtree Interval Covering

Given an arbitrary interval [start, end), `FindSubtrees` returns two
valid subtrees that efficiently cover it (§4.5). The second subtree may
be empty when the interval has at most one element:

```go
left, right, _ := mtc.FindSubtrees(5, 13)
...
draft-06 removed the dedicated pruning section; retention of long-expired
entries is now a property of the log serving protocol (see
[MTC-TLOG](https://c2sp.org/mtc-tlog) and [TLOG-TILES](https://c2sp.org/tlog-tiles)).
The log still supports dropping the data for long-expired entries while
...
| `1.3.6.1.4.1.44363.47.3` | `id-rdna-trustAnchorID` | Issuer DN attribute type |
| `1.3.6.1.4.1.44363.47.4` | `id-pe-mtcCertificationAuthority-SHA256` | MTC CA extension |

The issuer DN attribute value is a `RELATIVE-OID` holding the CA's trust
anchor ID (§5.1). These experimental OIDs live under the
`1.3.6.1.4.1.44363.47` arc kindly donated by Cloudflare, and will be
replaced with IANA-assigned OIDs when the specification is finalized.
...
- **§4** Subtrees — definition, inclusion proofs, consistency proofs, interval covering (empty subtrees are valid; the covering function always returns two subtrees)
- **§5** Issuance Logs — log structure, entries (`MTCLogEntry`), cosigners, CA cosigners
- **§6** Certificates — certificate format, standalone certificates, landmark-relative certificates (landmarks carry a tree size and expiration time)
- **§7** Relying Parties — trust anchors, signature verification, cosigner policies, trusted subtrees, revocation
- **Appendix C** Test Vectors — accumulated subtree test vectors (C.1) and large-tree subtree validity vectors (C.2)

Not implemented (out of scope or dependent on external protocols):
- §8 Use in TLS (trust anchor ID negotiation)
- §9 ACME Extensions
- §5.2.2 Log serving protocol (e.g. [tlog-tiles](https://c2sp.org/tlog-tiles), profiled for MTC in [mtc-tlog](https://c2sp.org/mtc-tlog))
...
- [Merkle Tree Certificates with Tiled Transparency Logs (MTC-TLOG)](https://c2sp.org/mtc-tlog)
...
- [2026-07-06] Update implementation and documentation to draft-ietf-plants-merkle-tree-certs-05.
- [2026-09-21] Update implementation and documentation to draft-ietf-plants-merkle-tree-certs-06.
