...
| `entry.go` | §5.2.1 | Log entry encoding (`MerkleTreeCertEntry`) with extensions |
...
Long-expired entries can be pruned from the log (§5.2.3):
...
- **Appendix C** Subtree Test Vectors — accumulated test vectors for subtree algorithms

Not implemented (out of scope or dependent on external protocols):
- §8 Use in TLS (trust anchor ID negotiation)
- §9 ACME Extensions
- §5.2.2 Log serving protocol (e.g. tlog-tiles)
